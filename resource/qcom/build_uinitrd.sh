...
cp -a ${TMP_KERNEL}/lib/modules ./lib/modules
cp -a ${FIRMWARE_DIR}/* ./lib/firmware

echo "Stage 4: Pack Ramdisk..."
cd ${TMP_RAMDISK}
find . | cpio -o -H newc | gzip >${TMP_DIR}/ramdisk.cpio.gz

echo "Stage 5: Build uInitrd..."
mkimage -A arm64 -O linux -T ramdisk -C gzip -n "uInitrd" -d ${TMP_DIR}/ramdisk.cpio.gz ${PWD_DIR}/uInitrd
